---
title: "Automatic Schedule Conflict Detection"
...
Sessions marked `isVirtual: true` — or those without `day`, `startTime`, or `endTime` — are always excluded from conflict evaluation in both `resolveConflicts` and `useConflictResolver`. They are returned in a separate bucket and rendered outside the time grid entirely.

```typescript
// Both paths apply this filter before the O(n²) comparison loop
const schedulable = sessions.filter(
  (s) => !s.isVirtual && s.day && s.startTime && s.endTime
);
```
